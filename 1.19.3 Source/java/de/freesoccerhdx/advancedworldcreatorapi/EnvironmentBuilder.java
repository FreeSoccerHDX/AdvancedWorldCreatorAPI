...
public class EnvironmentBuilder implements Keyed {
    ...
    public EnvironmentBuilder(NamespacedKey namespacedKey) {
        ...
        ResourceKey<DimensionType> resourceKeyDimension = ResourceKey.create(Registries.DIMENSION_TYPE, new ResourceLocation(namespacedKey.getNamespace(),namespacedKey.getKey()));
        WritableRegistry<DimensionType> registryDimensions = (WritableRegistry<DimensionType>) craftServer.getHandle().getServer().registryAccess().registryOrThrow(Registries.DIMENSION_TYPE);

        DimensionType dimType = registryDimensions.get(resourceKeyDimension);
