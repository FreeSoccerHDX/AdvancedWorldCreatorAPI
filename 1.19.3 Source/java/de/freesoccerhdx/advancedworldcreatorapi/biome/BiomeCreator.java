...
public class BiomeCreator implements Keyed {
    ...
    public CustomBiome createBiome(boolean replace) {
        ...
        WritableRegistry<Biome> biomeRegistry = CreateWorld.getWritetableRegistry(registryAccess, Registries.BIOME);
        ResourceKey<Biome> resourceKey = CreateWorld.createResourceKey(getKey(), Registries.BIOME);
    ...
    private Biome createBiomeBase() {
        ...
        for(BiomeFeatureType decorationType : biomeCarvers.keySet()){
            for(BiomeCarvers biomeCarver : biomeCarvers.get(decorationType)){
                generationSetting.addCarver(decorationType.getFeature(), Holder.direct(biomeCarver.getCarver()));
            }
        }
        ...
        if(music != null) {
            bf.backgroundMusic(new Music(Holder.direct(CraftSound.getSoundEffect(music.getSound())),music.getMinDelay(),music.getMaxDelay(),music.isReplacingCurrentMusic()));
        }
        BiomeCaveSound caveSound = getCaveSound();
        if(caveSound != null) {
            bf.ambientAdditionsSound(new AmbientAdditionsSettings(Holder.direct(CraftSound.getSoundEffect(caveSound.getSound())),caveSound.getTickChance()));
        }

        BiomeCaveSoundSettings caveSoundSettings = getCaveSoundSettings();
        if(caveSoundSettings != null){
            bf.ambientMoodSound(new AmbientMoodSettings(Holder.direct(CraftSound.getSoundEffect(caveSoundSettings.getSound())),caveSoundSettings.getTickDelay(),caveSoundSettings.getBlockSearchExtent(),caveSoundSettings.getOffset()));
        }
        Sound ambientSound = getAmbientSound();
        if(ambientSound != null) {
            bf.ambientLoopSound(Holder.direct(CraftSound.getSoundEffect(ambientSound)));
        }
        ...
        .precipitation(getPrecipitation().getPrecipitation())
