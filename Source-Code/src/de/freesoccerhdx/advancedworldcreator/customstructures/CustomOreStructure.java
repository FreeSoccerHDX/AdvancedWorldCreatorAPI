...
public class CustomOreStructure implements CustomStructure{
	...
	public CustomOreStructure(String name, int minY, int maxY, int oresize, int perchunk, Material oremat, Material target) {
		this(name, minY, maxY, oresize, perchunk, ((CraftBlockData) oremat.createBlockData()).getState().getBlock(), ((CraftBlockData) target.createBlockData()).getState().getBlock());
	}
	...
	public WorldGenFeatureConfigured<?, ?> create() {
		...
		WorldGenFeatureConfigured<?, ?> customore = a(name,
				oregenerator
					.b(new WorldGenFeatureOreConfiguration(rule, ore_material.getBlockData(), oresize))
					//.a(VerticalAnchor.a(minimum), VerticalAnchor.a(maximum))
					.a(range)
					.a()
					.b(perchunk)
			); 
