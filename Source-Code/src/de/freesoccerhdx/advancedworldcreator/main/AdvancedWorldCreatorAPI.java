...
public class AdvancedWorldCreatorAPI extends JavaPlugin implements Listener {
	...
	private static HashMap<NamespacedKey,ResourceKey<BiomeBase>> vanillabiomes = new HashMap<>();
	...
	public void onCmd(PlayerCommandPreprocessEvent e) {
		...
		if(cmd.equalsIgnoreCase("/testworld")) {
			...
			advancedcreator.setDecorationBlacklisted(WorldDecoration.RAW_GENERATION, false); //RAW_GENERATION
			advancedcreator.setDecorationBlacklisted(WorldDecoration.LAKES, false); //LAKES
			advancedcreator.setDecorationBlacklisted(WorldDecoration.LOCAL_MODIFICATIONS, false); //LOCAL_MODIFICATIONS
			advancedcreator.setDecorationBlacklisted(WorldDecoration.UNDERGROUND_STRUCTURES, false); //UNDERGROUND_STRUCTURES
			advancedcreator.setDecorationBlacklisted(WorldDecoration.SURFACE_STRUCTURES, false); //SURFACE_STRUCTURES
			advancedcreator.setDecorationBlacklisted(WorldDecoration.STRONGHOLDS, false); //STRONGHOLDS
			advancedcreator.setDecorationBlacklisted(WorldDecoration.UNDERGROUND_ORES, false); //UNDERGROUND_ORES
			advancedcreator.setDecorationBlacklisted(WorldDecoration.UNDERGROUND_DECORATION, false); //UNDERGROUND_DECORATION
			advancedcreator.setDecorationBlacklisted(WorldDecoration.VEGETAL_DECORATION, false); //VEGETAL_DECORATION
			advancedcreator.setDecorationBlacklisted(WorldDecoration.TOP_LAYER_MODIFICATION, false); //TOP_LAYER_MODIFICATION
	    ...
	    //    Block fluid_material = Blocks.A;
	        ...
	        //CraftBlockData.newData(null, cmd).getState()
			...
			CustomDimensionSettings dimensionmanager = new CustomDimensionSettings(
					...
					true, true, true, 
					-32, 256, 256, Tag.INFINIBURN_END,
					WorldEffects.OVERWORLD, 1.0f);
	...
	public static ResourceKey<BiomeBase> getNMSBiome(Biome bukkit_biome){
		
		NamespacedKey nk = bukkit_biome.getKey();
		
		return vanillabiomes.get(nk);
	}
		...
		try {
			...
			for(Field f : Biomes.class.getFields()) {
				...
				if(biome instanceof ResourceKey) {
					ResourceKey<BiomeBase> realbiome = (ResourceKey<BiomeBase>) biome;
					
					NamespacedKey nk = new NamespacedKey(realbiome.a().getNamespace(), realbiome.a().getKey());
					
					vanillabiomes.put(nk, realbiome);
					
				}
			}
			main.getLogger().log(Level.INFO, "[AdvancedWorldCreatorAPI] All Vanilla Biomes: " + vanillabiomes.size());
		}catch(Exception ex) {
			ex.printStackTrace();
		}
	}
	public static List<ResourceKey<BiomeBase>> getAllVanillaBiomes(){
		return ImmutableList.copyOf(vanillabiomes.values());
	}
	...
	public static RegisteredCustomBiome registerCustomBiome(CustomBiome cb, boolean replace) {
		try {			
			...
			try {
				if(Class.forName("org.bukkit.craftbukkit.libs.it.unimi.dsi.fastutil.ints.Int2ObjectMap") != null) {
					...
					bbb.put(maxid+1, resourcekeybiome);
				}
			}catch (Exception e) {
				it.unimi.dsi.fastutil.ints.Int2ObjectMap<ResourceKey<BiomeBase>> bbb = ( it.unimi.dsi.fastutil.ints.Int2ObjectMap<ResourceKey<BiomeBase>>) c_obj;
	...
	protected static NewDummyChunkGeneratorAbstract createChunkGenerator(AdvancedCreator creator, WorldChunkManager wcm, IRegistry<BiomeBase> var0, IRegistry<GeneratorSettingBase> iregistry_generatorsettingbase) {
//		GeneratorSettingBase gsb = GeneratorSettingBase.i();
		...
		if(creator.hasCustomWorldSetting()) {
			...
			settingbase = iregistry_generatorsettingbase.d(creator.getWorldSetting().get());
		}
		
		
		Map<StructureGenerator<?>, Boolean> custommap = new HashMap<>();
        for(StructureGenerator<?> sg : ss.a().keySet()) {
        	custommap.put(sg, true);
        }
        if(custommap.size() > 0) {
        	try {
        		Field f = WorldChunkManager.class.getDeclaredField("b");
        		
        		FieldUtils.writeField(f, wcm, custommap, true);
        		
        		//FieldUtils.writeDeclaredField(wcm, "b", custommap, true);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	//    Preconditions.checkState(!console.R.isEmpty(), "Cannot create additional worlds on STARTUP");
	    ...
	    try {
	    	...
	    	try {
	    		Method method = MinecraftServer.class.getDeclaredMethod("aZ", null);
	    		
	    		if(method != null) {
	    			resourcemanager = (IResourceManager) method.invoke((MinecraftServer)console, null);
	    		}
			...
			if(resourcemanager == null) {
				try {
					Method method = MinecraftServer.class.getDeclaredMethod("getResourceManager", null);
					resourcemanager = (IResourceManager) method.invoke((MinecraftServer)console, null);
					//resourcemanager = console.getResourceManager();
				}catch (Exception e) {
					e.printStackTrace();
				}
			}
	    ...
	    RegistryReadOps<NBTBase> registryreadops = RegistryReadOps.a((DynamicOps)DynamicOpsNBT.a, resourcemanager, (IRegistryCustom)console.l);
	...
	//    Bukkit.broadcastMessage("§c AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
	    
	    worlddata.c(true); // disable initing of the world (causes normaly large lags )
	    ...
	    WorldServer internal = new WorldServer(console, 
	    		console.az, 
	    		worldSession, 
	    		worlddata, 
	    		worldKey, 
	    		dimensionsettings.get(), 
	    		(console.getServer()).L.create(11), 
	    		chunkgenerator,
	    		worlddata.getGeneratorSettings().isDebugWorld(),
	    		j,
	    		(environment == World.Environment.NORMAL) ? (List)immutableList : (List)ImmutableList.of(),
	    		true, 
	    		environment,
	    		generator,
	    		null);
	    
	//   Bukkit.broadcastMessage("§c BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB");
	 ...
	 //   Bukkit.broadcastMessage("§c CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC");
	    
	    console.initWorld(internal, worlddata, worlddata, worlddata.getGeneratorSettings());

	//    Bukkit.broadcastMessage("§c DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD");
